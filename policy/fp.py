...
from network.fp_net import FPActionEncoder, FPAttention, FPCritic
...
class FP:
    def __init__(self, args):
        ...
        self.eval_critic = FPCritic(critic_input_shape, self.args)
        self.target_critic = FPCritic(critic_input_shape, self.args)

        self.encoder = FPActionEncoder(self.n_actions * self.n_agents, self.args)
        self.attention = FPAttention(self.args)

        self.target_encoder = FPActionEncoder(self.n_actions * self.n_agents, self.args)
        self.target_attention = FPAttention(self.args)
        ...
        if self.args.load_model:
            if os.path.exists(self.model_dir + '/rnn_params.pkl'):
                ...
                path_fp = self.model_dir + '/critic_params.pkl'
                map_location = 'cuda:0' if self.args.cuda else 'cpu'
                self.eval_rnn.load_state_dict(torch.load(path_rnn, map_location=map_location))
                self.eval_critic.load_state_dict(torch.load(path_fp, map_location=map_location))
                print('Successfully load the model: {} and {}'.format(path_rnn, path_fp))
            else:
        ...
        self.critic_parameters = list(self.eval_critic.parameters())+list(self.encoder.parameters())+list(self.attention.parameters())
    ...
    def _get_critic_input_shape(self):
        ...
        input_shape += self.args.encoder_dim
    ...
    def _get_q_values(self, batch, max_episode_len):
        ...
        for transition_idx in range(max_episode_len):
            ...
            if transition_idx == 0:  # 如果是第一条经验，就让前一个动作为0向量
                u_onehot_history = torch.zeros_like(batch['u_onehot'][:, transition_idx]).unsqueeze(1)
            else:
                u_onehot_history = batch['u_onehot'][:, :transition_idx]

            # 添加之前动作的attention
            u_onehot_history = u_onehot_history.view((episode_num, u_onehot_history.shape[1], -1))
            u_onehot_next_history = batch['u_onehot'][:, :transition_idx + 1]
            u_onehot_next_history = u_onehot_next_history.view((episode_num, u_onehot_next_history.shape[1], -1))

            if self.args.cuda:
                inputs = inputs.cuda()
                inputs_next = inputs_next.cuda()
                u_onehot_history = u_onehot_history.cuda()
                u_onehot_next_history = u_onehot_next_history.cuda()

            enc_output, s0 = self.encoder(u_onehot_history)
            # c = [episode_num, n_agents, args.encoder_dim]
            c = self.attention(s0, enc_output).repeat(1, self.n_agents, 1)
            c = c.view(episode_num*self.n_agents, -1)
            inputs = torch.cat([inputs, c], dim=1)
            
            enc_output_next, s0_next = self.target_encoder(u_onehot_next_history)
            c_next = self.target_attention(s0_next, enc_output_next).repeat(1, self.n_agents, 1)
            c_next = c_next.view(episode_num*self.n_agents, -1)
            inputs_next = torch.cat([inputs_next, c_next], dim=1)
    ...
    def _train_critic(self, batch, max_episode_len, train_step):
        ...
        if train_step > 0 and train_step % self.args.target_update_cycle == 0:
            self.target_critic.load_state_dict(self.eval_critic.state_dict())
            self.target_attention.load_state_dict(self.attention.state_dict())
    ...
    def save_model(self, train_step):
        ...
        torch.save(self.encoder.state_dict(),  self.model_dir + '/' + num + '_encoder_params.pkl')
        torch.save(self.attention.state_dict(),  self.model_dir + '/' + num + '_attention_params.pkl')